...
export class MasterTeacherService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<MasterTeacher | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], MasterTeacher, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<MasterTeacher>): Promise<MasterTeacher | undefined> {
        return await this.masterTeacherRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<MasterTeacher>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[MasterTeacher[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, MasterTeacher, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(masterTeacher: MasterTeacher): Promise<MasterTeacher | undefined> {
        const element = await this.masterTeacherRepository.save(masterTeacher);
        return element;
    }

    async update(masterTeacher: MasterTeacher): Promise<MasterTeacher | undefined> {
        const element = await this.masterTeacherRepository.save(masterTeacher);
        return element;
    }

    async delete(masterTeacher: MasterTeacher): Promise<MasterTeacher | undefined> {
        const element = await this.masterTeacherRepository.remove(masterTeacher);
        return element;
    }
}
