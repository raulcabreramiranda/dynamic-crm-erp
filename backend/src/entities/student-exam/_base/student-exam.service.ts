...
export class StudentExamService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<StudentExam | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], StudentExam, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<StudentExam>): Promise<StudentExam | undefined> {
        return await this.studentExamRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<StudentExam>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[StudentExam[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, StudentExam, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(studentExam: StudentExam): Promise<StudentExam | undefined> {
        const element = await this.studentExamRepository.save(studentExam);
        return element;
    }

    async update(studentExam: StudentExam): Promise<StudentExam | undefined> {
        const element = await this.studentExamRepository.save(studentExam);
        return element;
    }

    async delete(studentExam: StudentExam): Promise<StudentExam | undefined> {
        const element = await this.studentExamRepository.remove(studentExam);
        return element;
    }
}
