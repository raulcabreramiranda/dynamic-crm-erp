...
export class EssayService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Essay | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Essay, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Essay>): Promise<Essay | undefined> {
        return await this.essayRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Essay>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[Essay[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Essay, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(essay: Essay): Promise<Essay | undefined> {
        const element = await this.essayRepository.save(essay);
        return element;
    }

    async update(essay: Essay): Promise<Essay | undefined> {
        const element = await this.essayRepository.save(essay);
        return element;
    }

    async delete(essay: Essay): Promise<Essay | undefined> {
        const element = await this.essayRepository.remove(essay);
        return element;
    }
}
