...
export class QuestionService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Question | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Question, { id: this.request['user']?.['id'] }, selectColumns);
    ...
    async findAndCount(
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Question, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(question: Question): Promise<Question | undefined> {
        const element = await this.questionRepository.save(question);
        return element;
    }
    ...
    async delete(question: Question): Promise<Question | undefined> {
        const element = await this.questionRepository.remove(question);
        return element;
    }
}
