...
export class QuestionLevel4Service {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<QuestionLevel4 | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], QuestionLevel4, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<QuestionLevel4>): Promise<QuestionLevel4 | undefined> {
        return await this.questionLevel4Repository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<QuestionLevel4>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[QuestionLevel4[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, QuestionLevel4, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(questionLevel4: QuestionLevel4): Promise<QuestionLevel4 | undefined> {
        const element = await this.questionLevel4Repository.save(questionLevel4);
        return element;
    }

    async update(questionLevel4: QuestionLevel4): Promise<QuestionLevel4 | undefined> {
        const element = await this.questionLevel4Repository.save(questionLevel4);
        return element;
    }

    async delete(questionLevel4: QuestionLevel4): Promise<QuestionLevel4 | undefined> {
        const element = await this.questionLevel4Repository.remove(questionLevel4);
        return element;
    }
}
