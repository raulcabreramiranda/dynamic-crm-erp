...
export class JorneyGaleryService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<JorneyGalery | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], JorneyGalery, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<JorneyGalery>): Promise<JorneyGalery | undefined> {
        return await this.jorneyGaleryRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<JorneyGalery>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[JorneyGalery[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, JorneyGalery, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(jorneyGalery: JorneyGalery): Promise<JorneyGalery | undefined> {
        const element = await this.jorneyGaleryRepository.save(jorneyGalery);
        return element;
    }

    async update(jorneyGalery: JorneyGalery): Promise<JorneyGalery | undefined> {
        const element = await this.jorneyGaleryRepository.save(jorneyGalery);
        return element;
    }

    async delete(jorneyGalery: JorneyGalery): Promise<JorneyGalery | undefined> {
        const element = await this.jorneyGaleryRepository.remove(jorneyGalery);
        return element;
    }
}
