...
export class TrainingEnvironmentComponentService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<TrainingEnvironmentComponent | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], TrainingEnvironmentComponent, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<TrainingEnvironmentComponent>): Promise<TrainingEnvironmentComponent | undefined> {
        return await this.trainingEnvironmentComponentRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<TrainingEnvironmentComponent>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[TrainingEnvironmentComponent[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, TrainingEnvironmentComponent, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(trainingEnvironmentComponent: TrainingEnvironmentComponent): Promise<TrainingEnvironmentComponent | undefined> {
        const element = await this.trainingEnvironmentComponentRepository.save(trainingEnvironmentComponent);
        return element;
    }

    async update(trainingEnvironmentComponent: TrainingEnvironmentComponent): Promise<TrainingEnvironmentComponent | undefined> {
        const element = await this.trainingEnvironmentComponentRepository.save(trainingEnvironmentComponent);
        return element;
    }

    async delete(trainingEnvironmentComponent: TrainingEnvironmentComponent): Promise<TrainingEnvironmentComponent | undefined> {
        const element = await this.trainingEnvironmentComponentRepository.remove(trainingEnvironmentComponent);
        return element;
    }
}
