...
export class ExamCardReadService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ExamCardRead | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ExamCardRead, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ExamCardRead>): Promise<ExamCardRead | undefined> {
        return await this.examCardReadRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ExamCardRead>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ExamCardRead[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ExamCardRead, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(examCardRead: ExamCardRead): Promise<ExamCardRead | undefined> {
        const element = await this.examCardReadRepository.save(examCardRead);
        return element;
    }

    async update(examCardRead: ExamCardRead): Promise<ExamCardRead | undefined> {
        const element = await this.examCardReadRepository.save(examCardRead);
        return element;
    }

    async delete(examCardRead: ExamCardRead): Promise<ExamCardRead | undefined> {
        const element = await this.examCardReadRepository.remove(examCardRead);
        return element;
    }
}
