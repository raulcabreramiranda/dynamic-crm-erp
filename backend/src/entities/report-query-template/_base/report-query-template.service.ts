...
export class ReportQueryTemplateService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ReportQueryTemplate | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ReportQueryTemplate, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ReportQueryTemplate>): Promise<ReportQueryTemplate | undefined> {
        return await this.reportQueryTemplateRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ReportQueryTemplate>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ReportQueryTemplate[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ReportQueryTemplate, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(reportQueryTemplate: ReportQueryTemplate): Promise<ReportQueryTemplate | undefined> {
        const element = await this.reportQueryTemplateRepository.save(reportQueryTemplate);
        return element;
    }

    async update(reportQueryTemplate: ReportQueryTemplate): Promise<ReportQueryTemplate | undefined> {
        const element = await this.reportQueryTemplateRepository.save(reportQueryTemplate);
        return element;
    }

    async delete(reportQueryTemplate: ReportQueryTemplate): Promise<ReportQueryTemplate | undefined> {
        const element = await this.reportQueryTemplateRepository.remove(reportQueryTemplate);
        return element;
    }
}
