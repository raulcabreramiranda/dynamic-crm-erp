...
export class CernePlataformService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<CernePlataform | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], CernePlataform, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<CernePlataform>): Promise<CernePlataform | undefined> {
        return await this.cernePlataformRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<CernePlataform>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[CernePlataform[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, CernePlataform, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(cernePlataform: CernePlataform): Promise<CernePlataform | undefined> {
        const element = await this.cernePlataformRepository.save(cernePlataform);
        return element;
    }

    async update(cernePlataform: CernePlataform): Promise<CernePlataform | undefined> {
        const element = await this.cernePlataformRepository.save(cernePlataform);
        return element;
    }

    async delete(cernePlataform: CernePlataform): Promise<CernePlataform | undefined> {
        const element = await this.cernePlataformRepository.remove(cernePlataform);
        return element;
    }
}
