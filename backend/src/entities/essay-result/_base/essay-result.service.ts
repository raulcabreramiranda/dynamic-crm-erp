...
export class EssayResultService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<EssayResult | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], EssayResult, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<EssayResult>): Promise<EssayResult | undefined> {
        return await this.essayResultRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<EssayResult>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[EssayResult[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, EssayResult, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(essayResult: EssayResult): Promise<EssayResult | undefined> {
        const element = await this.essayResultRepository.save(essayResult);
        return element;
    }

    async update(essayResult: EssayResult): Promise<EssayResult | undefined> {
        const element = await this.essayResultRepository.save(essayResult);
        return element;
    }

    async delete(essayResult: EssayResult): Promise<EssayResult | undefined> {
        const element = await this.essayResultRepository.remove(essayResult);
        return element;
    }
}
