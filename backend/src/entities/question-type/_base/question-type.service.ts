...
export class QuestionTypeService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<QuestionType | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], QuestionType, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<QuestionType>): Promise<QuestionType | undefined> {
        return await this.questionTypeRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<QuestionType>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[QuestionType[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, QuestionType, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(questionType: QuestionType): Promise<QuestionType | undefined> {
        const element = await this.questionTypeRepository.save(questionType);
        return element;
    }

    async update(questionType: QuestionType): Promise<QuestionType | undefined> {
        const element = await this.questionTypeRepository.save(questionType);
        return element;
    }

    async delete(questionType: QuestionType): Promise<QuestionType | undefined> {
        const element = await this.questionTypeRepository.remove(questionType);
        return element;
    }
}
