...
export class EssayResultCommentService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<EssayResultComment | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], EssayResultComment, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<EssayResultComment>): Promise<EssayResultComment | undefined> {
        return await this.essayResultCommentRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<EssayResultComment>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[EssayResultComment[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, EssayResultComment, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(essayResultComment: EssayResultComment): Promise<EssayResultComment | undefined> {
        const element = await this.essayResultCommentRepository.save(essayResultComment);
        return element;
    }

    async update(essayResultComment: EssayResultComment): Promise<EssayResultComment | undefined> {
        const element = await this.essayResultCommentRepository.save(essayResultComment);
        return element;
    }

    async delete(essayResultComment: EssayResultComment): Promise<EssayResultComment | undefined> {
        const element = await this.essayResultCommentRepository.remove(essayResultComment);
        return element;
    }
}
