...
export class QuestionTextService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<QuestionText | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], QuestionText, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<QuestionText>): Promise<QuestionText | undefined> {
        return await this.questionTextRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<QuestionText>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[QuestionText[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, QuestionText, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(questionText: QuestionText): Promise<QuestionText | undefined> {
        const element = await this.questionTextRepository.save(questionText);
        return element;
    }

    async update(questionText: QuestionText): Promise<QuestionText | undefined> {
        const element = await this.questionTextRepository.save(questionText);
        return element;
    }

    async delete(questionText: QuestionText): Promise<QuestionText | undefined> {
        const element = await this.questionTextRepository.remove(questionText);
        return element;
    }
}
