...
export class AdminUserSuperProService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<AdminUserSuperPro | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], AdminUserSuperPro, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<AdminUserSuperPro>): Promise<AdminUserSuperPro | undefined> {
        return await this.adminUserSuperProRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<AdminUserSuperPro>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[AdminUserSuperPro[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, AdminUserSuperPro, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(adminUserSuperPro: AdminUserSuperPro): Promise<AdminUserSuperPro | undefined> {
        const element = await this.adminUserSuperProRepository.save(adminUserSuperPro);
        return element;
    }

    async update(adminUserSuperPro: AdminUserSuperPro): Promise<AdminUserSuperPro | undefined> {
        const element = await this.adminUserSuperProRepository.save(adminUserSuperPro);
        return element;
    }

    async delete(adminUserSuperPro: AdminUserSuperPro): Promise<AdminUserSuperPro | undefined> {
        const element = await this.adminUserSuperProRepository.remove(adminUserSuperPro);
        return element;
    }
}
