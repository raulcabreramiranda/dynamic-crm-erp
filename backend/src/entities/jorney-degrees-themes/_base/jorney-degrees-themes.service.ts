...
export class JorneyDegreesThemesService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<JorneyDegreesThemes | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], JorneyDegreesThemes, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<JorneyDegreesThemes>): Promise<JorneyDegreesThemes | undefined> {
        return await this.jorneyDegreesThemesRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<JorneyDegreesThemes>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[JorneyDegreesThemes[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, JorneyDegreesThemes, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(jorneyDegreesThemes: JorneyDegreesThemes): Promise<JorneyDegreesThemes | undefined> {
        const element = await this.jorneyDegreesThemesRepository.save(jorneyDegreesThemes);
        return element;
    }

    async update(jorneyDegreesThemes: JorneyDegreesThemes): Promise<JorneyDegreesThemes | undefined> {
        const element = await this.jorneyDegreesThemesRepository.save(jorneyDegreesThemes);
        return element;
    }

    async delete(jorneyDegreesThemes: JorneyDegreesThemes): Promise<JorneyDegreesThemes | undefined> {
        const element = await this.jorneyDegreesThemesRepository.remove(jorneyDegreesThemes);
        return element;
    }
}
