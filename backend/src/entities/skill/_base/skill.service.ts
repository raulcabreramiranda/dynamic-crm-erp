...
export class SkillService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Skill | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Skill, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Skill>): Promise<Skill | undefined> {
        return await this.skillRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Skill>,
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Skill, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(skill: Skill): Promise<Skill | undefined> {
        const element = await this.skillRepository.save(skill);
        return element;
    }

    async update(skill: Skill): Promise<Skill | undefined> {
        const element = await this.skillRepository.save(skill);
        return element;
    }

    async delete(skill: Skill): Promise<Skill | undefined> {
        const element = await this.skillRepository.remove(skill);
        return element;
    }
}
