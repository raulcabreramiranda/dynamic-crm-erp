...
export class JorneyGalerySectionSubjectService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<JorneyGalerySectionSubject | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], JorneyGalerySectionSubject, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<JorneyGalerySectionSubject>): Promise<JorneyGalerySectionSubject | undefined> {
        return await this.jorneyGalerySectionSubjectRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<JorneyGalerySectionSubject>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[JorneyGalerySectionSubject[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, JorneyGalerySectionSubject, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(jorneyGalerySectionSubject: JorneyGalerySectionSubject): Promise<JorneyGalerySectionSubject | undefined> {
        const element = await this.jorneyGalerySectionSubjectRepository.save(jorneyGalerySectionSubject);
        return element;
    }

    async update(jorneyGalerySectionSubject: JorneyGalerySectionSubject): Promise<JorneyGalerySectionSubject | undefined> {
        const element = await this.jorneyGalerySectionSubjectRepository.save(jorneyGalerySectionSubject);
        return element;
    }

    async delete(jorneyGalerySectionSubject: JorneyGalerySectionSubject): Promise<JorneyGalerySectionSubject | undefined> {
        const element = await this.jorneyGalerySectionSubjectRepository.remove(jorneyGalerySectionSubject);
        return element;
    }
}
