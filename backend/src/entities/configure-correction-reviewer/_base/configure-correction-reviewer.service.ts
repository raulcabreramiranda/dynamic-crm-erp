...
export class ConfigureCorrectionReviewerService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ConfigureCorrectionReviewer | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ConfigureCorrectionReviewer, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ConfigureCorrectionReviewer>): Promise<ConfigureCorrectionReviewer | undefined> {
        return await this.configureCorrectionReviewerRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ConfigureCorrectionReviewer>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ConfigureCorrectionReviewer[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ConfigureCorrectionReviewer, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(configureCorrectionReviewer: ConfigureCorrectionReviewer): Promise<ConfigureCorrectionReviewer | undefined> {
        const element = await this.configureCorrectionReviewerRepository.save(configureCorrectionReviewer);
        return element;
    }

    async update(configureCorrectionReviewer: ConfigureCorrectionReviewer): Promise<ConfigureCorrectionReviewer | undefined> {
        const element = await this.configureCorrectionReviewerRepository.save(configureCorrectionReviewer);
        return element;
    }

    async delete(configureCorrectionReviewer: ConfigureCorrectionReviewer): Promise<ConfigureCorrectionReviewer | undefined> {
        const element = await this.configureCorrectionReviewerRepository.remove(configureCorrectionReviewer);
        return element;
    }
}
