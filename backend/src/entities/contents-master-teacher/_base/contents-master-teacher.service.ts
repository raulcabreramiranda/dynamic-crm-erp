...
export class ContentsMasterTeacherService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ContentsMasterTeacher | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ContentsMasterTeacher, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ContentsMasterTeacher>): Promise<ContentsMasterTeacher | undefined> {
        return await this.contentsMasterTeacherRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ContentsMasterTeacher>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ContentsMasterTeacher[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ContentsMasterTeacher, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(contentsMasterTeacher: ContentsMasterTeacher): Promise<ContentsMasterTeacher | undefined> {
        const element = await this.contentsMasterTeacherRepository.save(contentsMasterTeacher);
        return element;
    }

    async update(contentsMasterTeacher: ContentsMasterTeacher): Promise<ContentsMasterTeacher | undefined> {
        const element = await this.contentsMasterTeacherRepository.save(contentsMasterTeacher);
        return element;
    }

    async delete(contentsMasterTeacher: ContentsMasterTeacher): Promise<ContentsMasterTeacher | undefined> {
        const element = await this.contentsMasterTeacherRepository.remove(contentsMasterTeacher);
        return element;
    }
}
