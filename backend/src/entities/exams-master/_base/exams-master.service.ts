...
export class ExamsMasterService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ExamsMaster | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ExamsMaster, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ExamsMaster>): Promise<ExamsMaster | undefined> {
        return await this.examsMasterRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ExamsMaster>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ExamsMaster[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ExamsMaster, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(examsMaster: ExamsMaster): Promise<ExamsMaster | undefined> {
        const element = await this.examsMasterRepository.save(examsMaster);
        return element;
    }

    async update(examsMaster: ExamsMaster): Promise<ExamsMaster | undefined> {
        const element = await this.examsMasterRepository.save(examsMaster);
        return element;
    }

    async delete(examsMaster: ExamsMaster): Promise<ExamsMaster | undefined> {
        const element = await this.examsMasterRepository.remove(examsMaster);
        return element;
    }
}
