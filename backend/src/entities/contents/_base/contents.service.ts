...
export class ContentsService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Contents | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Contents, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Contents>): Promise<Contents | undefined> {
        return await this.contentsRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Contents>,
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Contents, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(contents: Contents): Promise<Contents | undefined> {
        const element = await this.contentsRepository.save(contents);
        return element;
    }

    async update(contents: Contents): Promise<Contents | undefined> {
        const element = await this.contentsRepository.save(contents);
        return element;
    }

    async delete(contents: Contents): Promise<Contents | undefined> {
        const element = await this.contentsRepository.remove(contents);
        return element;
    }
}
