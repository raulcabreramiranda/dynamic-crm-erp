...
export class MasterService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Master | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Master, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Master>): Promise<Master | undefined> {
        return await this.masterRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Master>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[Master[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Master, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(master: Master): Promise<Master | undefined> {
        const element = await this.masterRepository.save(master);
        return element;
    }

    async update(master: Master): Promise<Master | undefined> {
        const element = await this.masterRepository.save(master);
        return element;
    }

    async delete(master: Master): Promise<Master | undefined> {
        const element = await this.masterRepository.remove(master);
        return element;
    }
}
