...
export class FontService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Font | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Font, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Font>): Promise<Font | undefined> {
        return await this.fontRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Font>,
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Font, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(font: Font): Promise<Font | undefined> {
        const element = await this.fontRepository.save(font);
        return element;
    }

    async update(font: Font): Promise<Font | undefined> {
        const element = await this.fontRepository.save(font);
        return element;
    }

    async delete(font: Font): Promise<Font | undefined> {
        const element = await this.fontRepository.remove(font);
        return element;
    }
}
