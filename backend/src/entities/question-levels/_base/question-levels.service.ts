...
export class QuestionLevelsService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<QuestionLevels | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], QuestionLevels, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<QuestionLevels>): Promise<QuestionLevels | undefined> {
        return await this.questionLevelsRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<QuestionLevels>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[QuestionLevels[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, QuestionLevels, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(questionLevels: QuestionLevels): Promise<QuestionLevels | undefined> {
        const element = await this.questionLevelsRepository.save(questionLevels);
        return element;
    }

    async update(questionLevels: QuestionLevels): Promise<QuestionLevels | undefined> {
        const element = await this.questionLevelsRepository.save(questionLevels);
        return element;
    }

    async delete(questionLevels: QuestionLevels): Promise<QuestionLevels | undefined> {
        const element = await this.questionLevelsRepository.remove(questionLevels);
        return element;
    }
}
