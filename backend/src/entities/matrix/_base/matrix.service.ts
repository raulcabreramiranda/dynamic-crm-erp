...
export class MatrixService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Matrix | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Matrix, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Matrix>): Promise<Matrix | undefined> {
        return await this.matrixRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Matrix>,
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Matrix, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(matrix: Matrix): Promise<Matrix | undefined> {
        const element = await this.matrixRepository.save(matrix);
        return element;
    }

    async update(matrix: Matrix): Promise<Matrix | undefined> {
        const element = await this.matrixRepository.save(matrix);
        return element;
    }

    async delete(matrix: Matrix): Promise<Matrix | undefined> {
        const element = await this.matrixRepository.remove(matrix);
        return element;
    }
}
