...
export class ReportByQueryService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ReportByQuery | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ReportByQuery, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ReportByQuery>): Promise<ReportByQuery | undefined> {
        return await this.reportByQueryRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ReportByQuery>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ReportByQuery[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ReportByQuery, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(reportByQuery: ReportByQuery): Promise<ReportByQuery | undefined> {
        const element = await this.reportByQueryRepository.save(reportByQuery);
        return element;
    }

    async update(reportByQuery: ReportByQuery): Promise<ReportByQuery | undefined> {
        const element = await this.reportByQueryRepository.save(reportByQuery);
        return element;
    }

    async delete(reportByQuery: ReportByQuery): Promise<ReportByQuery | undefined> {
        const element = await this.reportByQueryRepository.remove(reportByQuery);
        return element;
    }
}
