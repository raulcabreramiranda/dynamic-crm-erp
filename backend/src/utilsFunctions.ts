...
import { SkillItemRepository } from './entities/skill-item/skill-item.repository';
...
export const rIntegration = async (essay: Essay, skillItemRepository: SkillItemRepository): Promise<SkillItem[]> => {
  const R = require('r-integration');

  /**  
   **** INSTALL EvoApp Loader ****
   ...
   library(keras)
   install_keras()
   "/usr/bin/Rscript" -e "source('/home/ec2-user/mestre-evo-nextjs/EvoApp/Loader.R') ; print(predict_evo_red(text='Testando'))"
*/
  const text = essay.typedText;
  const result = R.callMethod(process.env.NODE_SERVER_EVO_APP_LOADER, 'predict_evo_red', {
    text: text.split('`').join('\\`').split(`"`).join(`\\"`).split(`'`).join(`\\'`),
  });

  console.log(result[0]);
  const notes = JSON.parse(result[0]);
  console.log({ notes });

  const iaResponse = [notes[0], notes[2], notes[4], notes[6], notes[8]];
  const skillItems: SkillItem[] = [];

  for (const _skill in iaResponse) {
    const points = iaResponse[_skill];
    const skillItem: SkillItem = await skillItemRepository.findOne({
      relations: ['skill'],
      where: {
        points: Equal(points),
        skill: { id: Equal(+_skill + 1) },
      },
    });
