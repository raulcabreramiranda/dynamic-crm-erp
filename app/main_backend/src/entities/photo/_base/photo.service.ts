...
export class PhotoService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Photo | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Photo, { id: this.request['user']?.['id'] }, selectColumns);
    ...
    async findAndCount(
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Photo, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(photo: Photo): Promise<Photo | undefined> {
        const element = await this.photoRepository.save(photo);
        return element;
    }
    ...
    async delete(photo: Photo): Promise<Photo | undefined> {
        const element = await this.photoRepository.remove(photo);
        return element;
    }
}
