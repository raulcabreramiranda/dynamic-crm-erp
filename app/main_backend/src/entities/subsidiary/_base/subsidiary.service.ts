...
export class SubsidiaryService {
    ...
    constructor(@Inject(REQUEST) protected readonly request: Request, @Inject('SUBSIDIARY_REPOSITORY') protected subsidiaryRepository: Repository<Subsidiary>) {}

    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Subsidiary | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount2({
            ...
            repository: this.subsidiaryRepository,
            Entity: Subsidiary,
            userRequest: {
    ...
    async findByfields(options: FindOneOptions<Subsidiary>): Promise<Subsidiary | undefined> {
        return await this.subsidiaryRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Subsidiary>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[Subsidiary[], number]> {
    ...
    async save(subsidiary: Subsidiary): Promise<Subsidiary | undefined> {
        const element = await this.subsidiaryRepository.save(subsidiary);
        return element;
    }

    async update(subsidiary: Subsidiary): Promise<Subsidiary | undefined> {
        const element = await this.subsidiaryRepository.save(subsidiary);
        return element;
    }

    async delete(subsidiary: Subsidiary): Promise<Subsidiary | undefined> {
        const element = await this.subsidiaryRepository.remove(subsidiary);
        return element;
    }
}
