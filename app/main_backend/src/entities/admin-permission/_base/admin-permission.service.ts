...
export class AdminPermissionService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<AdminPermission | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], AdminPermission, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<AdminPermission>): Promise<AdminPermission | undefined> {
        return await this.adminPermissionRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<AdminPermission>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[AdminPermission[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, AdminPermission, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(adminPermission: AdminPermission): Promise<AdminPermission | undefined> {
        const element = await this.adminPermissionRepository.save(adminPermission);
        return element;
    }

    async update(adminPermission: AdminPermission): Promise<AdminPermission | undefined> {
        const element = await this.adminPermissionRepository.save(adminPermission);
        return element;
    }

    async delete(adminPermission: AdminPermission): Promise<AdminPermission | undefined> {
        const element = await this.adminPermissionRepository.remove(adminPermission);
        return element;
    }
}
