...
export class CompanyService {
    ...
    constructor(@Inject(REQUEST) protected readonly request: Request, @Inject('COMPANY_REPOSITORY') protected companyRepository: Repository<Company>) {}

    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Company | undefined> {
        const options: any = {};
    ...
    async findByfields(options: FindOneOptions<Company>): Promise<Company | undefined> {
    ...
    async save(company: Company): Promise<Company | undefined> {
        const element = await this.companyRepository.save(company);
        return element;
    }

    async update(company: Company): Promise<Company | undefined> {
        const element = await this.companyRepository.save(company);
        return element;
    }

    async delete(company: Company): Promise<Company | undefined> {
        const element = await this.companyRepository.remove(company);
        return element;
    }
}
