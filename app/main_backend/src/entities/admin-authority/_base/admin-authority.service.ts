...
export class AdminAuthorityService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<AdminAuthority | undefined> {
        ...
        const result = await getManyAndCount(options, [], AdminAuthority, { id: this.request['user']?.['id'] }, selectColumns);
    ...
    async findAndCount(
        ...
        options.where = where;
        return await getManyAndCount(options, filters, AdminAuthority, { id: this.request['user']?.['id'] }, selectColumns);
    }
