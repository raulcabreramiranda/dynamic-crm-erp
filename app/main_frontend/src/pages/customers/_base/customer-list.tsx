...
import { apiGetList, apiGetEntityForm, apiGetEntityView, apiDeleteEntity } from 'src/pages/customers/_base/customer-services';

import { ICustomer } from 'src/pages/customers/_base/customer-model';
...
const ListTable = ({}: any) => {
    ...
    const deleteEntityModal = (entity: ICustomer) => {
    ...
    const openUpdateModal = (entity: ICustomer) => {
        if (typeof entity.id === 'number' && entity?.id > 0) {
    ...
    const openViewModal = (entity: ICustomer) => {
        if (typeof entity.id === 'number' && entity?.id > 0) {
            const handleSuccess = (response: IApiResponseProps) => {
    ...
    return (
        <>
            {entityList && entityList.filter && entityList.filter((v: any) => typeof v.deletedAt === 'undefined' || v.deletedAt === null).length > 0 ? (
                <div id="customer-table-list" className="table-list">
                    <TableContainer>
                        <Table>
                            <TableHead>
                                <TableHeadRow>
                                    ...
                                    .map((customer: any, i: number) => (
                                        <TableBodyRow tableRowIndex={i} key={`entity-${i}`}>
                                            <TableBodyCell id="commercialName-cell" align={'left'}>
                                                <TableText name={'commercialName'} entityView={customer} />
                                            </TableBodyCell>

                                            <TableBodyCell id="corporateName-cell" align={'left'}>
                                                <TableText name={'corporateName'} entityView={customer} />
                                            </TableBodyCell>

                                            <TableBodyCell id="cnpj-cell" align={'left'}>
                                                <TableText name={'cnpj'} entityView={customer} />
                                            </TableBodyCell>

                                            <TableBodyCell id="cpf-cell" align={'left'}>
                                                <TableText name={'cpf'} entityView={customer} />
                                            </TableBodyCell>

                                            <TableBodyCell id="telephone-cell" align={'left'}>
                                                <TableText name={'telephone'} entityView={customer} />
                                            </TableBodyCell>

                                            <TableBodyCell id="email-cell" align={'left'}>
                                                <TableText name={'email'} entityView={customer} />
                                            </TableBodyCell>

                                            <TableBodyCell align={'right'}>
                                                <div className="btn-group flex-btn-group-container">
                                                    {hasAnyAuthority(null, ['customer'], 'view') ? (
                                                        <Button color="primary" size="small" onClick={() => openViewModal(customer)} isLink={false} icon={'eye'}></Button>
                                                    ) : (
                                                        <></>
                                                    )}
                                                    {hasAnyAuthority(null, ['customer'], 'edit') ? (
                                                        <Button color="primary" size="small" onClick={() => openUpdateModal(customer)} isLink={false} icon={'pencil'}></Button>
                                                    ) : (
                                                        <></>
                                                    )}
                                                    {hasAnyAuthority(null, ['customer'], 'canDelete') ? (
                                                        <Button color="primary" size="small" onClick={() => deleteEntityModal(customer)} isLink={false} icon={'trash'}></Button>
                                                    ) : (
                                                        <></>
                                                    )}{' '}
                                                </div>
