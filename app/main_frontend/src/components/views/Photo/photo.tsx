...
import { apiGetList, apiGetEntityForm, apiGetEntityView, apiUpdateEntity, apiNewEntity, apiDeleteEntity } from 'src/components/services/Photo/photo-services';

import { IPhoto, IPhotoFilters } from 'src/components/models/Photo/photo-model';
import FormView from 'src/components/views/Photo/photo-view';
import FormUpdate from 'src/components/views/Photo/photo-form';
import ListTable, { IEntityListSort } from 'src/components/views/Photo/photo-list';
import FilterList from 'src/components/views/Photo/photo-filter';

import EntityContext, { IReloadList } from 'src/components/contexts/Photo/photo-context';
...
function ModalView() {
    ...
    const handleClose = () => {
        setEntityView({} as IPhoto);
    };

    return (
        <Dialog isOpen={true} onClose={handleClose}>
            <DialogTitle onClose={handleClose}>Detalhe do Photo</DialogTitle>
            <DialogContent>
                <FormView />
            </DialogContent>
            <DialogActions>
...
function ModalUpdate() {
    ...
    return (
        <Dialog isOpen={true} onClose={handleClose}>
            <DialogTitle onClose={handleClose}>{!!entityEdit.id && entityEdit?.id > 0 ? 'Atualizar Photo' : 'Criar Photo'}</DialogTitle>
            <DialogContent>
...
const MUITable = ({ baseFilters, baseEntity, startList }: Props) => {
    ...
    const [entityEdit, setEntityEdit] = useState<IPhoto>({});
    const [entityView, setEntityView] = useState<IPhoto>({});
