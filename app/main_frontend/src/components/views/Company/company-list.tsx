...
import { apiGetList, apiGetEntityForm, apiGetEntityView, apiDeleteEntity } from 'src/components/services/Company/company-services';

import { ICompany } from 'src/components/models/Company/company-model';
...
const ListTable = ({}: any) => {
    ...
    const deleteEntityModal = (entity: ICompany) => {
    ...
    const openViewModal = (entity: ICompany) => {
        if (typeof entity.id === 'number' && entity?.id > 0) {
            const handleSuccess = (response: IApiResponseProps) => {
    ...
    return (
        <>
            {entityList && entityList.filter && entityList.filter((v: any) => typeof v.deletedAt === 'undefined' || v.deletedAt === null).length > 0 ? (
                <div id="company-table-list" className="table-list">
                    <TableContainer>
                        <Table>
                            <TableHead>
                                <TableHeadRow>
                                    ...
                                    .map((company: any, i: number) => (
                                        <TableBodyRow tableRowIndex={i} key={`entity-${i}`}>
                                            <TableBodyCell id="commercialName-cell" align={'left'}>
                                                <TableText name={'commercialName'} entityView={company} />
                                            </TableBodyCell>

                                            <TableBodyCell id="corporateName-cell" align={'left'}>
                                                <TableText name={'corporateName'} entityView={company} />
                                            </TableBodyCell>

                                            <TableBodyCell id="cnpj-cell" align={'left'}>
                                                <TableText name={'cnpj'} entityView={company} />
                                            </TableBodyCell>

                                            <TableBodyCell id="telephone-cell" align={'left'}>
                                                <TableText name={'telephone'} entityView={company} />
                                            </TableBodyCell>

                                            <TableBodyCell id="email-cell" align={'left'}>
                                                <TableText name={'email'} entityView={company} />
                                            </TableBodyCell>
      ...
      />
                ...
                <div className="alert alert-warning">
                    <Translate contentKey="company.home.notFound">No Companies found</Translate>
                </div>
            ) : (
                <div />
            )}
