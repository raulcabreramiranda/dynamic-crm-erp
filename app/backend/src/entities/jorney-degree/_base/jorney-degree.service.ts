...
export class JorneyDegreeService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<JorneyDegree | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], JorneyDegree, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<JorneyDegree>): Promise<JorneyDegree | undefined> {
        return await this.jorneyDegreeRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<JorneyDegree>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[JorneyDegree[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, JorneyDegree, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(jorneyDegree: JorneyDegree): Promise<JorneyDegree | undefined> {
        const element = await this.jorneyDegreeRepository.save(jorneyDegree);
        return element;
    }

    async update(jorneyDegree: JorneyDegree): Promise<JorneyDegree | undefined> {
        const element = await this.jorneyDegreeRepository.save(jorneyDegree);
        return element;
    }

    async delete(jorneyDegree: JorneyDegree): Promise<JorneyDegree | undefined> {
        const element = await this.jorneyDegreeRepository.remove(jorneyDegree);
        return element;
    }
}
