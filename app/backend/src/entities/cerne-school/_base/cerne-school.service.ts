...
export class CerneSchoolService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<CerneSchool | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], CerneSchool, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<CerneSchool>): Promise<CerneSchool | undefined> {
        return await this.cerneSchoolRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<CerneSchool>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[CerneSchool[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, CerneSchool, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(cerneSchool: CerneSchool): Promise<CerneSchool | undefined> {
        const element = await this.cerneSchoolRepository.save(cerneSchool);
        return element;
    }

    async update(cerneSchool: CerneSchool): Promise<CerneSchool | undefined> {
        const element = await this.cerneSchoolRepository.save(cerneSchool);
        return element;
    }

    async delete(cerneSchool: CerneSchool): Promise<CerneSchool | undefined> {
        const element = await this.cerneSchoolRepository.remove(cerneSchool);
        return element;
    }
}
