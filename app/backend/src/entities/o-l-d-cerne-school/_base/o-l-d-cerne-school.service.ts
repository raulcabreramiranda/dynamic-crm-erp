...
export class OLDCerneSchoolService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<OLDCerneSchool | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], OLDCerneSchool, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<OLDCerneSchool>): Promise<OLDCerneSchool | undefined> {
        return await this.oLDCerneSchoolRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<OLDCerneSchool>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[OLDCerneSchool[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, OLDCerneSchool, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(oLDCerneSchool: OLDCerneSchool): Promise<OLDCerneSchool | undefined> {
        const element = await this.oLDCerneSchoolRepository.save(oLDCerneSchool);
        return element;
    }

    async update(oLDCerneSchool: OLDCerneSchool): Promise<OLDCerneSchool | undefined> {
        const element = await this.oLDCerneSchoolRepository.save(oLDCerneSchool);
        return element;
    }

    async delete(oLDCerneSchool: OLDCerneSchool): Promise<OLDCerneSchool | undefined> {
        const element = await this.oLDCerneSchoolRepository.remove(oLDCerneSchool);
        return element;
    }
}
