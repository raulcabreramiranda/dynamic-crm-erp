...
export class DisciplineService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Discipline | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Discipline, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Discipline>): Promise<Discipline | undefined> {
        return await this.disciplineRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Discipline>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[Discipline[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Discipline, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(discipline: Discipline): Promise<Discipline | undefined> {
        const element = await this.disciplineRepository.save(discipline);
        return element;
    }

    async update(discipline: Discipline): Promise<Discipline | undefined> {
        const element = await this.disciplineRepository.save(discipline);
        return element;
    }

    async delete(discipline: Discipline): Promise<Discipline | undefined> {
        const element = await this.disciplineRepository.remove(discipline);
        return element;
    }
}
