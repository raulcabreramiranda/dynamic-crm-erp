...
export class JorneyService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Jorney | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Jorney, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Jorney>): Promise<Jorney | undefined> {
        return await this.jorneyRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Jorney>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[Jorney[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Jorney, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(jorney: Jorney): Promise<Jorney | undefined> {
        const element = await this.jorneyRepository.save(jorney);
        return element;
    }

    async update(jorney: Jorney): Promise<Jorney | undefined> {
        const element = await this.jorneyRepository.save(jorney);
        return element;
    }

    async delete(jorney: Jorney): Promise<Jorney | undefined> {
        const element = await this.jorneyRepository.remove(jorney);
        return element;
    }
}
