...
export class CerneClassService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<CerneClass | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], CerneClass, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<CerneClass>): Promise<CerneClass | undefined> {
        return await this.cerneClassRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<CerneClass>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[CerneClass[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, CerneClass, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(cerneClass: CerneClass): Promise<CerneClass | undefined> {
        const element = await this.cerneClassRepository.save(cerneClass);
        return element;
    }

    async update(cerneClass: CerneClass): Promise<CerneClass | undefined> {
        const element = await this.cerneClassRepository.save(cerneClass);
        return element;
    }

    async delete(cerneClass: CerneClass): Promise<CerneClass | undefined> {
        const element = await this.cerneClassRepository.remove(cerneClass);
        return element;
    }
}
