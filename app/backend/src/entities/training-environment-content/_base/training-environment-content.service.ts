...
export class TrainingEnvironmentContentService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<TrainingEnvironmentContent | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], TrainingEnvironmentContent, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<TrainingEnvironmentContent>): Promise<TrainingEnvironmentContent | undefined> {
        return await this.trainingEnvironmentContentRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<TrainingEnvironmentContent>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[TrainingEnvironmentContent[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, TrainingEnvironmentContent, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(trainingEnvironmentContent: TrainingEnvironmentContent): Promise<TrainingEnvironmentContent | undefined> {
        const element = await this.trainingEnvironmentContentRepository.save(trainingEnvironmentContent);
        return element;
    }

    async update(trainingEnvironmentContent: TrainingEnvironmentContent): Promise<TrainingEnvironmentContent | undefined> {
        const element = await this.trainingEnvironmentContentRepository.save(trainingEnvironmentContent);
        return element;
    }

    async delete(trainingEnvironmentContent: TrainingEnvironmentContent): Promise<TrainingEnvironmentContent | undefined> {
        const element = await this.trainingEnvironmentContentRepository.remove(trainingEnvironmentContent);
        return element;
    }
}
