...
export class QuestionMatrixService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<QuestionMatrix | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], QuestionMatrix, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<QuestionMatrix>): Promise<QuestionMatrix | undefined> {
        return await this.questionMatrixRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<QuestionMatrix>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[QuestionMatrix[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, QuestionMatrix, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(questionMatrix: QuestionMatrix): Promise<QuestionMatrix | undefined> {
        const element = await this.questionMatrixRepository.save(questionMatrix);
        return element;
    }

    async update(questionMatrix: QuestionMatrix): Promise<QuestionMatrix | undefined> {
        const element = await this.questionMatrixRepository.save(questionMatrix);
        return element;
    }

    async delete(questionMatrix: QuestionMatrix): Promise<QuestionMatrix | undefined> {
        const element = await this.questionMatrixRepository.remove(questionMatrix);
        return element;
    }
}
