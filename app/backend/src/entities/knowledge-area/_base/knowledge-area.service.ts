...
export class KnowledgeAreaService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<KnowledgeArea | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], KnowledgeArea, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<KnowledgeArea>): Promise<KnowledgeArea | undefined> {
        return await this.knowledgeAreaRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<KnowledgeArea>,
        ...
        options.where = where;
        return await getManyAndCount(options, filters, KnowledgeArea, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(knowledgeArea: KnowledgeArea): Promise<KnowledgeArea | undefined> {
        const element = await this.knowledgeAreaRepository.save(knowledgeArea);
        return element;
    }

    async update(knowledgeArea: KnowledgeArea): Promise<KnowledgeArea | undefined> {
        const element = await this.knowledgeAreaRepository.save(knowledgeArea);
        return element;
    }

    async delete(knowledgeArea: KnowledgeArea): Promise<KnowledgeArea | undefined> {
        const element = await this.knowledgeAreaRepository.remove(knowledgeArea);
        return element;
    }
}
