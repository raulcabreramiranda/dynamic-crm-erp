...
export class ConfigureCorrectionService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ConfigureCorrection | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ConfigureCorrection, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ConfigureCorrection>): Promise<ConfigureCorrection | undefined> {
        return await this.configureCorrectionRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ConfigureCorrection>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ConfigureCorrection[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ConfigureCorrection, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(configureCorrection: ConfigureCorrection): Promise<ConfigureCorrection | undefined> {
        const element = await this.configureCorrectionRepository.save(configureCorrection);
        return element;
    }

    async update(configureCorrection: ConfigureCorrection): Promise<ConfigureCorrection | undefined> {
        const element = await this.configureCorrectionRepository.save(configureCorrection);
        return element;
    }

    async delete(configureCorrection: ConfigureCorrection): Promise<ConfigureCorrection | undefined> {
        const element = await this.configureCorrectionRepository.remove(configureCorrection);
        return element;
    }
}
