...
export class TrainingEnvironmentService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<TrainingEnvironment | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], TrainingEnvironment, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<TrainingEnvironment>): Promise<TrainingEnvironment | undefined> {
        return await this.trainingEnvironmentRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<TrainingEnvironment>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[TrainingEnvironment[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, TrainingEnvironment, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(trainingEnvironment: TrainingEnvironment): Promise<TrainingEnvironment | undefined> {
        const element = await this.trainingEnvironmentRepository.save(trainingEnvironment);
        return element;
    }

    async update(trainingEnvironment: TrainingEnvironment): Promise<TrainingEnvironment | undefined> {
        const element = await this.trainingEnvironmentRepository.save(trainingEnvironment);
        return element;
    }

    async delete(trainingEnvironment: TrainingEnvironment): Promise<TrainingEnvironment | undefined> {
        const element = await this.trainingEnvironmentRepository.remove(trainingEnvironment);
        return element;
    }
}
