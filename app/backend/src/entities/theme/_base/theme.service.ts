...
export class ThemeService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Theme | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Theme, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Theme>): Promise<Theme | undefined> {
        return await this.themeRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Theme>,
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Theme, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(theme: Theme): Promise<Theme | undefined> {
        const element = await this.themeRepository.save(theme);
        return element;
    }

    async update(theme: Theme): Promise<Theme | undefined> {
        const element = await this.themeRepository.save(theme);
        return element;
    }

    async delete(theme: Theme): Promise<Theme | undefined> {
        const element = await this.themeRepository.remove(theme);
        return element;
    }
}
