...
export class ExamTemplateService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ExamTemplate | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ExamTemplate, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ExamTemplate>): Promise<ExamTemplate | undefined> {
        return await this.examTemplateRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ExamTemplate>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ExamTemplate[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ExamTemplate, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(examTemplate: ExamTemplate): Promise<ExamTemplate | undefined> {
        const element = await this.examTemplateRepository.save(examTemplate);
        return element;
    }

    async update(examTemplate: ExamTemplate): Promise<ExamTemplate | undefined> {
        const element = await this.examTemplateRepository.save(examTemplate);
        return element;
    }

    async delete(examTemplate: ExamTemplate): Promise<ExamTemplate | undefined> {
        const element = await this.examTemplateRepository.remove(examTemplate);
        return element;
    }
}
