...
export class ExamConfigureApplicationService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ExamConfigureApplication | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ExamConfigureApplication, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ExamConfigureApplication>): Promise<ExamConfigureApplication | undefined> {
        return await this.examConfigureApplicationRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ExamConfigureApplication>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ExamConfigureApplication[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ExamConfigureApplication, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(examConfigureApplication: ExamConfigureApplication): Promise<ExamConfigureApplication | undefined> {
        const element = await this.examConfigureApplicationRepository.save(examConfigureApplication);
        return element;
    }

    async update(examConfigureApplication: ExamConfigureApplication): Promise<ExamConfigureApplication | undefined> {
        const element = await this.examConfigureApplicationRepository.save(examConfigureApplication);
        return element;
    }

    async delete(examConfigureApplication: ExamConfigureApplication): Promise<ExamConfigureApplication | undefined> {
        const element = await this.examConfigureApplicationRepository.remove(examConfigureApplication);
        return element;
    }
}
