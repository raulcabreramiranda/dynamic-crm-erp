...
export class ReportCategoryService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ReportCategory | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ReportCategory, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ReportCategory>): Promise<ReportCategory | undefined> {
        return await this.reportCategoryRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ReportCategory>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ReportCategory[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ReportCategory, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(reportCategory: ReportCategory): Promise<ReportCategory | undefined> {
        const element = await this.reportCategoryRepository.save(reportCategory);
        return element;
    }

    async update(reportCategory: ReportCategory): Promise<ReportCategory | undefined> {
        const element = await this.reportCategoryRepository.save(reportCategory);
        return element;
    }

    async delete(reportCategory: ReportCategory): Promise<ReportCategory | undefined> {
        const element = await this.reportCategoryRepository.remove(reportCategory);
        return element;
    }
}
