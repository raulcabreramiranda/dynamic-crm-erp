...
export class DisciplineSuperProService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<DisciplineSuperPro | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], DisciplineSuperPro, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<DisciplineSuperPro>): Promise<DisciplineSuperPro | undefined> {
        return await this.disciplineSuperProRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<DisciplineSuperPro>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[DisciplineSuperPro[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, DisciplineSuperPro, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(disciplineSuperPro: DisciplineSuperPro): Promise<DisciplineSuperPro | undefined> {
        const element = await this.disciplineSuperProRepository.save(disciplineSuperPro);
        return element;
    }

    async update(disciplineSuperPro: DisciplineSuperPro): Promise<DisciplineSuperPro | undefined> {
        const element = await this.disciplineSuperProRepository.save(disciplineSuperPro);
        return element;
    }

    async delete(disciplineSuperPro: DisciplineSuperPro): Promise<DisciplineSuperPro | undefined> {
        const element = await this.disciplineSuperProRepository.remove(disciplineSuperPro);
        return element;
    }
}
