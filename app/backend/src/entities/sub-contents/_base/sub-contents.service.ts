...
export class SubContentsService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<SubContents | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], SubContents, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<SubContents>): Promise<SubContents | undefined> {
        return await this.subContentsRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<SubContents>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[SubContents[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, SubContents, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(subContents: SubContents): Promise<SubContents | undefined> {
        const element = await this.subContentsRepository.save(subContents);
        return element;
    }

    async update(subContents: SubContents): Promise<SubContents | undefined> {
        const element = await this.subContentsRepository.save(subContents);
        return element;
    }

    async delete(subContents: SubContents): Promise<SubContents | undefined> {
        const element = await this.subContentsRepository.remove(subContents);
        return element;
    }
}
