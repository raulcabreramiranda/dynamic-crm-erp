...
export class ExamCardService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ExamCard | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ExamCard, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ExamCard>): Promise<ExamCard | undefined> {
        return await this.examCardRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ExamCard>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ExamCard[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ExamCard, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(examCard: ExamCard): Promise<ExamCard | undefined> {
        const element = await this.examCardRepository.save(examCard);
        return element;
    }

    async update(examCard: ExamCard): Promise<ExamCard | undefined> {
        const element = await this.examCardRepository.save(examCard);
        return element;
    }

    async delete(examCard: ExamCard): Promise<ExamCard | undefined> {
        const element = await this.examCardRepository.remove(examCard);
        return element;
    }
}
