...
export class AdminAuditEntityService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<AdminAuditEntity | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], AdminAuditEntity, { id: this.request['user']?.['id'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<AdminAuditEntity>): Promise<AdminAuditEntity | undefined> {
        return await this.adminAuditEntityRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<AdminAuditEntity>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[AdminAuditEntity[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, AdminAuditEntity, { id: this.request['user']?.['id'] }, selectColumns);
    }

    async save(adminAuditEntity: AdminAuditEntity): Promise<AdminAuditEntity | undefined> {
        const element = await this.adminAuditEntityRepository.save(adminAuditEntity);
        return element;
    }

    async update(adminAuditEntity: AdminAuditEntity): Promise<AdminAuditEntity | undefined> {
        const element = await this.adminAuditEntityRepository.save(adminAuditEntity);
        return element;
    }

    async delete(adminAuditEntity: AdminAuditEntity): Promise<AdminAuditEntity | undefined> {
        const element = await this.adminAuditEntityRepository.remove(adminAuditEntity);
        return element;
    }
}
