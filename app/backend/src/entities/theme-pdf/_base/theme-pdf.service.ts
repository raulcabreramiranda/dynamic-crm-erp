...
export class ThemePdfService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ThemePdf | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ThemePdf, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ThemePdf>): Promise<ThemePdf | undefined> {
        return await this.themePdfRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ThemePdf>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ThemePdf[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ThemePdf, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(themePdf: ThemePdf): Promise<ThemePdf | undefined> {
        const element = await this.themePdfRepository.save(themePdf);
        return element;
    }

    async update(themePdf: ThemePdf): Promise<ThemePdf | undefined> {
        const element = await this.themePdfRepository.save(themePdf);
        return element;
    }

    async delete(themePdf: ThemePdf): Promise<ThemePdf | undefined> {
        const element = await this.themePdfRepository.remove(themePdf);
        return element;
    }
}
