...
export class ExamTypeService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ExamType | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ExamType, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ExamType>): Promise<ExamType | undefined> {
        return await this.examTypeRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ExamType>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ExamType[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ExamType, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(examType: ExamType): Promise<ExamType | undefined> {
        const element = await this.examTypeRepository.save(examType);
        return element;
    }

    async update(examType: ExamType): Promise<ExamType | undefined> {
        const element = await this.examTypeRepository.save(examType);
        return element;
    }

    async delete(examType: ExamType): Promise<ExamType | undefined> {
        const element = await this.examTypeRepository.remove(examType);
        return element;
    }
}
