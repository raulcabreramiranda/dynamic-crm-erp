...
export class ExamService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<Exam | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], Exam, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<Exam>): Promise<Exam | undefined> {
        return await this.examRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<Exam>,
        ...
        options.where = where;
        return await getManyAndCount(options, filters, Exam, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(exam: Exam): Promise<Exam | undefined> {
        const element = await this.examRepository.save(exam);
        return element;
    }

    async update(exam: Exam): Promise<Exam | undefined> {
        const element = await this.examRepository.save(exam);
        return element;
    }

    async delete(exam: Exam): Promise<Exam | undefined> {
        const element = await this.examRepository.remove(exam);
        return element;
    }
}
