...
export class ExamsMasterKnowledgeAreaService {
    ...
    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<ExamsMasterKnowledgeArea | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount(options, [], ExamsMasterKnowledgeArea, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
        return result[1] > 0 ? result[0][0] : null;
    }

    async findByfields(options: FindOneOptions<ExamsMasterKnowledgeArea>): Promise<ExamsMasterKnowledgeArea | undefined> {
        return await this.examsMasterKnowledgeAreaRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<ExamsMasterKnowledgeArea>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[ExamsMasterKnowledgeArea[], number]> {
        ...
        options.where = where;
        return await getManyAndCount(options, filters, ExamsMasterKnowledgeArea, { id: this.request['user']?.['id'], whiteLabel: this.request['user']?.['whiteLabel'] }, selectColumns);
    }

    async save(examsMasterKnowledgeArea: ExamsMasterKnowledgeArea): Promise<ExamsMasterKnowledgeArea | undefined> {
        const element = await this.examsMasterKnowledgeAreaRepository.save(examsMasterKnowledgeArea);
        return element;
    }

    async update(examsMasterKnowledgeArea: ExamsMasterKnowledgeArea): Promise<ExamsMasterKnowledgeArea | undefined> {
        const element = await this.examsMasterKnowledgeAreaRepository.save(examsMasterKnowledgeArea);
        return element;
    }

    async delete(examsMasterKnowledgeArea: ExamsMasterKnowledgeArea): Promise<ExamsMasterKnowledgeArea | undefined> {
        const element = await this.examsMasterKnowledgeAreaRepository.remove(examsMasterKnowledgeArea);
        return element;
    }
}
