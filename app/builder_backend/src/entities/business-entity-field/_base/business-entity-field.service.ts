...
export class BusinessEntityFieldService {
    logger = new Logger('BusinessEntityFieldService');

    constructor(@Inject(REQUEST) protected readonly request: Request, @Inject('BUSINESSENTITYFIELD_REPOSITORY') protected businessEntityFieldRepository: Repository<BusinessEntityField>) {}

    async findById(id: string, selectFields?: string[], selectColumns?: string): Promise<BusinessEntityField | undefined> {
        const options: any = {};
        ...
        const result = await getManyAndCount2({
            ...
            repository: this.businessEntityFieldRepository,
            Entity: BusinessEntityField,
            userRequest: {
    ...
    async findByfields(options: FindOneOptions<BusinessEntityField>): Promise<BusinessEntityField | undefined> {
        return await this.businessEntityFieldRepository.findOne(options);
    }

    async findAndCount(
        options: FindManyOptions<BusinessEntityField>,
        filters?: Array<{ column: string; value: string; operation: string }>[],
        selectFields?: string[],
        selectColumns?: string,
    ): Promise<[BusinessEntityField[], number]> {
    ...
    async save(businessEntityField: BusinessEntityField): Promise<BusinessEntityField | undefined> {
        const element = await this.businessEntityFieldRepository.save(businessEntityField);
        return element;
    }

    async update(businessEntityField: BusinessEntityField): Promise<BusinessEntityField | undefined> {
        const element = await this.businessEntityFieldRepository.save(businessEntityField);
        return element;
    }

    async delete(businessEntityField: BusinessEntityField): Promise<BusinessEntityField | undefined> {
        const element = await this.businessEntityFieldRepository.remove(businessEntityField);
        return element;
    }
}
